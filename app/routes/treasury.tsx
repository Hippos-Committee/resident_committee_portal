...
export default function Treasury({ loaderData }: Route.ComponentProps) {
    const { selectedYear, expenses, income, balance, years, transactionCount, languages } = loaderData;
        ...
        <QRPanel
            ...
            title={
                <h2 className="text-3xl font-black text-gray-900 dark:text-white tracking-tight">
                    ...
                    {isInfoReel && <span className="text-lg text-gray-400 font-bold">{t("treasury.view_breakdown", { lng: "en" })}</span>}
                </h2>
            }
        />
    );
    ...
    const actionItems = [
        {
            ...
            labelSecondary: t("treasury.actions.breakdown", { lng: languages.secondary }),
        },
        ...(hasPermission("reimbursements:read") ? [{
            href: "/treasury/reimbursements",
            icon: "receipt_long",
            labelPrimary: t("treasury.actions.reimbursements", { lng: languages.primary }),
            labelSecondary: t("treasury.actions.reimbursements", { lng: languages.secondary }),
        }] : []),
        ...(canWrite ? [{
            href: "/treasury/new",
            icon: "add",
            labelPrimary: t("treasury.actions.add", { lng: languages.primary }),
            labelSecondary: t("treasury.actions.add", { lng: languages.secondary }),
        }] : []),
    ];
    ...
    return (
        <PageWrapper>
            <SplitLayout
                ...
                header={{
                    primary: t("treasury.title", { year: selectedYear, lng: languages.primary }),
                    secondary: t("treasury.title", { year: selectedYear, lng: languages.secondary })
                }}
            >
                    ...
                    {transactionCount > 0 ? (
                        <>
                            ...
                            <div className="inline-block bg-gray-100 dark:bg-gray-800 px-4 py-2 rounded-lg">
                                <p className="text-sm font-medium text-gray-600 dark:text-gray-400">
                                    {t("treasury.transactions_count", { count: transactionCount })} —
                        ...
                        <div className="text-center py-12">
                            ...
                            <p className="text-gray-400 dark:text-gray-500 mb-4">
                                {isInfoReel ? (
                                    <>
                                        {t("treasury.no_transactions_desc", { year: selectedYear, lng: "fi" })}
                                        <br />
                                        {t("treasury.no_transactions_desc", { year: selectedYear, lng: "en" })}
                                    </>
                                ) : (
