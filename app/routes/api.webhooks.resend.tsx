...
async function processEmailEvent(event: ResendEmailReceivedEvent) {
    if (event.type !== "email.received") {
        console.log(`[Resend Webhook] Ignoring non-email.received event: ${event.type}`);
    ...
    if (data.html) {
        console.log("[Resend Webhook] HTML content (first 200 chars):", data.html.substring(0, 200));
    }
    ...
    if (!purchaseId) {
        console.log("[Resend Webhook] No purchase ID found in to addresses:", data.to);
    ...
    try {
        ...
        console.log("[Resend Webhook] Full receiving API response:", JSON.stringify(emailData, null, 2));

        // Use text body preferably, then HTML, then subject as fallback
        emailContent = emailData.data?.text || emailData.data?.html || data.subject || "";
    } catch (fetchError) {
        console.error("[Resend Webhook] Failed to fetch email via receiving API:", fetchError);
        // Fall back to subject only
    }

    console.log(`[Resend Webhook] Email content received:`, emailContent.substring(0, 200));

    // Parse the reply content
    const decision = await parseReimbursementReply(emailContent);
    console.log(`[Resend Webhook] Parsed decision: ${decision} for purchase ${purchaseId}`);
    ...
    try {
        ...
        if (decision === "approved" || decision === "rejected") {
            ...
            if (linkedTransaction) {
                const newReimbursementStatus = decision === "approved" ? "approved" : "declined";
                const newTransactionStatus = decision === "approved" ? "complete" : "declined";
                await db.updateTransaction(linkedTransaction.id, {
                    reimbursementStatus: newReimbursementStatus,
                    status: newTransactionStatus
                });
                console.log(`[Resend Webhook] Updated transaction ${linkedTransaction.id} status to: ${newTransactionStatus}, reimbursementStatus to: ${newReimbursementStatus}`);
            }
        }

        console.log(`[Resend Webhook] Updated purchase ${purchaseId} with decision: ${decision}`);
